...
from app.auth.forms import LoginForm, RegistrationForm, ResetPasswordRequestForm, \
    ResetPasswordForm, ConfirmEmailForm
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user.email_registered:
            flash('You have not yet confirmed your account. Please check your email.', 'error')
            return redirect(url_for('auth.login'))
...
def resend_confirmation():
    ...
    if form.validate_on_submit():
        ...
        if user.email_registered:
            ...
            flash('You have already confirmed your account. Please login below.', 'success')
            return redirect(url_for('auth.login'))
    ...
    return render_template('auth/resend_token.html', title='Resend Confirmation Email', form=form)
