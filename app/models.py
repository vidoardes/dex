...
class User(UserMixin, db.Model):
    ...
    def verify_password_token(token):
        try:
            id = jwt.decode(token, current_app.config['SECRET_KEY'], algorithms=['HS256'])['password_token']
        except:
            return
...
class Pokemon(db.Model):
    ...
    name = db.Column(db.String(120), unique=True, default='', primary_key=True, nullable=False)
